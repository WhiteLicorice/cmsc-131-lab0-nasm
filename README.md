...
**Windows, 2026-08-07.** The echo added to `read_int` and `read_char`, and
...
**Linux, 2026-08-07, by continuous integration.** Everything above ran on
Windows. The ELF side of the echo needs a Linux link and a Linux run, neither
of which any machine here can perform, so `test.yml` gained a step that builds
and checks `readback` and the tagged push was watched to completion.

| Check | Result |
|---|---|
| `nasm -f elf32 -d ELF_TYPE readback.asm`, then `gcc -m32 ... -no-pie` | links, so `isatty` resolved undecorated the way the ELF `%define` intends |
| `make PROG=readback check` | `OK: readback matches readback.expected` |
| `readback.expected`, which was generated on Windows | matched here byte for byte, so the echo produces the same bytes on both platforms |
| `make check` on `skel`, from clean and again after `clean` | `OK` both times, unchanged |
| the executable-stack guard | passes, so the `.note.GNU-stack` addition survived the edits around it |
...
GitHub Actions was in a major outage when `v1.9` was pushed, so the push to
`main` scheduled no run at all and the tag push scheduled one only after the
outage cleared. Both point at the same commit and run the same workflow, so
nothing went unchecked. A missing run on `main` for this commit means the
outage rather than a skipped check.
